...
// ============================================================================
// Metrics (lightweight atomic counters)
// ============================================================================

#[cfg(feature = "web-api")]
static SSE_CONNECTIONS: AtomicU64 = AtomicU64::new(0);
#[cfg(feature = "web-api")]
...
struct AppState {
    ...
    infer_rate_limit: Arc<DashMap<String, (u32, std::time::Instant)>>,
}
...
pub async fn start_server(
    ...
    // Background task: clean up stale rate-limit entries every 5 minutes.
    {
        let rate_limit_map = state.infer_rate_limit.clone();
        tokio::spawn(async move {
            let mut ticker = interval(Duration::from_secs(300));
            loop {
                ticker.tick().await;
                let cutoff = Duration::from_secs(120);
                rate_limit_map.retain(|_, v: &mut (u32, std::time::Instant)| {
                    v.1.elapsed() < cutoff
                });
            }
        });
    ...
    // Background task: GC tracked requests older than 24 hours, every hour.
    {
        let tracker_map = state.tracker.status.clone();
        tokio::spawn(async move {
            let mut ticker = interval(Duration::from_secs(3600));
            loop {
                ticker.tick().await;
                let cutoff = Duration::from_secs(86400);
                tracker_map.retain(|_, v: &mut TrackedRequest| {
                    v.created_at.elapsed() < cutoff
                });
            }
        });
    }
...
async fn auth_middleware(
    ...
    // Auth disabled — allow all but warn, and add X-Auth-Disabled header.
    let Some(ref expected_key) = state.api_key else {
        warn!(request_path = %req_path, "auth disabled — allowing unauthenticated request");
        let mut response = next.run(req).await;
        response
            .headers_mut()
            .insert("x-auth-disabled", HeaderValue::from_static("true"));
        return response;
    };
    ...
    let token_valid = req
        ...
        .map(|token| token.as_bytes().ct_eq(expected_key.as_bytes()).into())
        .unwrap_or(false);
...
#[cfg(feature = "web-api")]
const INFER_RATE_LIMIT_MAX: u32 = 60;
...
async fn infer_handler(
    ...
    // Per-IP rate limiting: 60 requests per 60-second window.
    {
        ...
        if entry.0 > INFER_RATE_LIMIT_MAX {
            RATE_LIMIT_REJECTIONS.fetch_add(1, Ordering::Relaxed);
    ...
    tracing::info!(request_id = %request_id, old_status = ?"none", new_status = ?RequestStatus::Pending, "request_status_change");
    ...
    if let Some(mut tracked) = state.tracker.status.get_mut(&request_id) {
        let old_status = tracked.status.clone();
        tracked.status = RequestStatus::Processing;
        tracing::info!(request_id = %request_id, old_status = ?old_status, new_status = ?RequestStatus::Processing, "request_status_change");
    }
...
/// `GET /api/v1/metrics`  -  Lightweight atomic counter metrics as JSON.
///
/// # Panics
///
/// This function never panics.
#[cfg(feature = "web-api")]
async fn api_metrics_handler() -> Json<serde_json::Value> {
    Json(serde_json::json!({
        "sse_connections": SSE_CONNECTIONS.load(Ordering::Relaxed),
